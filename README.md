...
equivalent to the former two for real numbers). Since these operations are their own
...
indexing (`getindex`) with `Union{Integer, Range{<:Integer}}` (a.k.a slicing).
...
of overloading `reshape`, Strided.jl provides a separate method `sreshape` which returns a
`StridedView` over the same parent data, or throws an error if this is impossible.

## Broacasting and `map(reduce)`
...
that aims to avoid cache misses. Furthermore, if `Threads.nthreads() > 1` and the arrays
involved are sufficiently large, a multithreaded implementation will be used. This
matters in particular if some of the `StridedView`s involved have strides which are not
monotonously increasing, e.g. if `transpose`, `adjoint` or `permutedims` has been
applied. The fact that the latter also acts lazily (whereas it creates a copy of the data
